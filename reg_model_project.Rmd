...
ggpairs(movies2,columns = c(4,8,16,18))
...
```{r}
ggplot(data = movies,aes(x=runtime,y=audience_score)) + geom_point() + stat_smooth(method=lm) + xlab("Runtime") + ylab("Audience Score")
```
...
movies2 <- movies %>% filter(!is.na(runtime))
...
```{r}
ggplot(data = movies,aes(x=thtr_rel_month,y=audience_score)) + geom_point() + stat_smooth(method=lm) + xlab("Month of Release") + ylab("Audience Score")
...
```{r}
ggplot(data = movies,aes(x=critics_score,y=audience_score)) + geom_point() + stat_smooth(method=lm) + xlab("Critics Score") + ylab("Audience Score")
